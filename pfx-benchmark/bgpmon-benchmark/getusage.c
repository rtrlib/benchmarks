//Last modified: 28/10/11 21:30:54(CEST) by Fabian Holler
#include <stdlib.h> 
#include <sys/types.h>
#include <stdio.h>
#include <strings.h>
...
void calc_cpu_usage(const struct pstat* cur_usage, const struct pstat* last_usage, double* ucpu_usage, double* scpu_usage){
    *ucpu_usage = 100 * ((((cur_usage->utime_ticks + cur_usage->cutime_ticks) - (last_usage->utime_ticks + last_usage->cutime_ticks))) / (double)((cur_usage->cpu_total_time - last_usage->cpu_total_time)));
    *scpu_usage = 100 * ((((cur_usage->stime_ticks + cur_usage->cstime_ticks) - (last_usage->stime_ticks + last_usage->cstime_ticks))) / (double)((cur_usage->cpu_total_time - last_usage->cpu_total_time)));
}
